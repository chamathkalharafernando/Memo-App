...
router.post('/sign-in', async(req, res) =>{
    ...
    return res.send(express.response);
});
...
router.post('/add-memo', auth, async(req, res)=>{
    ...
    if(!error) return res.status(400).send('Not authorized to perform this function');
    user.memos.push({
        timeStamp:Date(),
...
router.post('/delete-memo', auth, async(req, res)=>{
    ...
    const user = await User.findById(req.user._id);
    if(!error) return res.status(401).send('Not authorized to perform this function');
